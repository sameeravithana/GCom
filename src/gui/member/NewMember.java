    ...
    if (memName.isEmpty()) {
        ...
        if (gs.get(groupName) > 0) {
            ...
            try {
                ...
                if (group != null) {
                    // Make the new member, leader of the group

                    Member m = new Member(memName, group);

                    try {
                        group.addMember(m);
                        igm.updateStatus(m, IMessage.TYPE_MESSAGE.UPDATESTATUS);
